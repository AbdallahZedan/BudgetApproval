...
sap.ui.define([
	...
	return BaseController.extend("ExtensionApproval.ExtensionApproval.controller.Detail", {
		...
		retiveWorkItem: function() {
			...
			oModel.read(workItemPath, {
				...
				error: function(error) {
					MessageToast.show(that.getTextFromResourceBundle("failureAllocReq"));
				}
			});
		},
		...
		onNodePress: function(oEvent) {
			var selectedNode = oEvent.getParameters().getBindingContext("workItemModel").getObject();
			if (selectedNode.laneId !== "0") {
				this._nodeDialog = sap.ui.xmlfragment(this.getView().getId(), "ExtensionApproval.ExtensionApproval.view.NodeDialog",
					this);
				this._nodeDialog.setBindingContext(oEvent.getParameters().getBindingContext("workItemModel"), "workItemModel");
		...
		retriveExtensionDetails: function(sRequestId, sLineId) {
			...
			oModel.read(extensionPath, {
				...
				error: function(error) {
					MessageToast.show(that.getTextFromResourceBundle("failureExtReq"));
		...
		onConfirmPress: function(oEvent) {
			...
			var oId = oEvent.getSource().mProperties.type,
				/*oEvent.getSource().getId().split('-')[5]*/
