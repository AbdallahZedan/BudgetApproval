...
sap.ui.define([
	...
	MessageBox, MessageToast, Sorter) {
	"use strict";
	...
	return BaseController.extend("ExtensionApproval.ExtensionApproval.controller.Master", {
		...
		onInit: function() {
			...
			this.getView().addEventDelegate({
				onBeforeFirstShow: function() {
					...
					// 	this._oListFilterState.aFilter = [new Filter("UserName", FilterOperator.EQ,
					...
					// 	];
					// this.username = this.getModel("FLP").getProperty("/id");
					// this.onBypassed();
		...
		retriveAllExtRequsts: function() {
			...
			var oModel = this.getOwnerComponent().getModel(),
				oFilters = [new Filter("UserName", FilterOperator.EQ,
						this.getOwnerComponent().getModel("FLP").getProperty("/id")),
					// 		this.getModel("FLP").getProperty("/id")),
					// 			"CIC2"),
					new Filter("Status", FilterOperator.EQ, "STARTED"),
					new Filter("WorkflowType", FilterOperator.EQ, "EXTENSION")
		...
		onSortPress: function(oEvent) {
			...
			this._sortFragment = sap.ui.xmlfragment(this.getView().getId(), "ExtensionApproval.ExtensionApproval.view.Sort", this);
...
// 			}
		...
		onSelectionChange: function(oEvent) {
			...
			if (selectedContexts.length > 0) {
				...
				// this.selectedIndex = selectedContexts[selectedContexts.length - 1].sPath.split('/')[2],
				status = oEvent.getParameter("listItem").mAggregations.firstStatus.mProperties.text;
			...
			if (this._oList.getMode() === "MultiSelect") {
				...
				switch (status) {
					case this.getTextFromResourceBundle("ApprovedStatus"):
						this._oList.setSelectedItem(oItem, false);
						MessageBox.error(this.getTextFromResourceBundle("slectApprovedItem"));
						return;
					case this.getTextFromResourceBundle("RejectedStatus"):
						this._oList.setSelectedItem(oItem, false);
				...
				// 	this._oList.setSelectedItem(oItem, false);
				// 	MessageBox.error(status === this.getResourceBundle("ApprovedStatus") ? this.getTextFromResourceBundle("slectApprovedItem") :
				// 		this.getTextFromResourceBundle("selectRejectedItem"));
				// 	return;
				// }
		...
		onItemPress: function(oEvent) {
			var selectedContexts = this._oList.getSelectedContexts();
			this.selectedIndex = oEvent.getParameter("listItem").getBindingContext("inboxModel").getPath().split('/')[2];
			this._showDetail(oEvent.getParameter("listItem") || oEvent.getSource(), this.selectedIndex);
		},

		onPressConfirmAll: function(oEvent) {

			var workItems = this.getAllSelectedItem(),
				buttonPressed = oEvent.getSource().mProperties.text,
				decision,
				decisionCode;
